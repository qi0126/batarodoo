...
class Session(http.Controller):
    ...
    def authenticate(self, db, login, password, base_location=None):
        ...
        user = u.browse(request.cr, SUPERUSER_ID, res['uid'])
        print user
        if user.session_id:
            session_file = 'werkzeug_%s.sess' % user.session_id
            path = os.path.join(root.session_store.path, session_file)
            if os.path.exists(path):
                os.remove(path)
        u.write(request.cr, SUPERUSER_ID, res['uid'], {'session_id': res['session_id']})
        return res
    # @http.route('/web/session/authenticate', type='json', auth="none")
    # def authenticate(self, db, login, password, base_location=None):
    #     request.session.authenticate(db, login, password)
